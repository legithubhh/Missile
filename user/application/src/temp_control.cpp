 ...
 *
 * It checks if the temperature is greater than 35 degrees Celsius. If the temperature is below 35 degrees Celsius for 1 second continuously,
 * it sets the IO port PI7 high. If the temperature is above 35 degrees Celsius, it sets the IO port PI7 low.
 *
 * @note This function relies on the temperature readings from the `temperature` object.（此段注释由Copilot自动生成，请根据实际情况修改。）
 */
void TempTask(void)
{
    // Code to check if the temperature is greater than 35
    if (temperature.GetTemp1() < 35.f || temperature.GetTemp2() < 35.f) {
